    ...
    <div class="flex gap-1 overflow-x-auto pb-px">
        @foreach ($adminLinks as $l)
            <a href="{{ route($l['route']) }}"
               class="whitespace-nowrap px-3 py-2 text-sm font-medium border-b-2 -mb-px {{ request()->routeIs($l['pattern']) ? 'border-gray-900 dark:border-white text-gray-900 dark:text-white' : 'border-transparent text-gray-500 hover:text-gray-800 dark:hover:text-gray-200' }}">
                {{ $l['label'] }}
            </a>
        @endforeach
    </div>
