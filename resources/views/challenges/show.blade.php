<x-app-layout>
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <a href="{{ route('challenges.index') }}" class="text-sm text-indigo-600 hover:underline">← Back to challenges</a>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            @if ($challenge->category)<x-badge>{{ $challenge->category->icon }} {{ $challenge->category->name }}</x-badge>@endif
        </div>
            ...
            @if ($mySubmission)
                <x-card class="mt-6 p-5">
                    ...
                    @if ($mySubmission->submission_url)
                        <a href="{{ $mySubmission->submission_url }}" target="_blank" class="mt-2 inline-flex text-sm text-indigo-600 hover:underline">{{ $mySubmission->submission_url }} ↗</a>
                    @endif
            ...
            <x-card class="mt-6 p-5">
                ...
                <form method="POST" action="{{ route('challenges.submit', $challenge) }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="url" name="submission_url" placeholder="https://…" class="block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <x-input-error :messages="$errors->get('submission_url')" class="mt-1" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">Notes (optional)</label>
                        <textarea name="notes" rows="3" class="block w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 text-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                    </div>
                    <button class="px-5 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700">Submit</button>
                </form>
            </x-card>
        @else
            <x-card class="mt-6 p-5 text-sm text-gray-600 dark:text-gray-300">
                <a href="{{ route('login') }}" class="text-indigo-600 font-medium hover:underline">Log in</a> to take on this challenge.
            </x-card>
        @endauth
    </div>
</x-app-layout>
